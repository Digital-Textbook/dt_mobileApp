...
const Onboarding = () =>{
    ...
    return(
        <SafeAreaView className="flex h-full items-center justify-between bg-[#F4F3F3]">
            ...
            ref={swiperRef}
            loop={false}
            dot={<View className="w-[10px] h-[10px] mx-1 mt-20 bg-[#D9D9D9] rounded-full"/>}
            activeDot={<View className="w-[10px] h-[10px] mx-1 mt-20 bg-[#225FA7] rounded-full"/>}
            ...
            >
                {onboarding.map((item)=> ( 
                    <View key={item.id} className="flex items-center justify-center p-2">
                        ...
                        <Text className="text-sm font-interMedium text-center text-black mx-7">{item.description}</Text>
